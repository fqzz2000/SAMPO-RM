...
def cbf(states,r_safe):
    return (states[:,0] - states[:,2])**2 - r_safe**2
...
class Next_state:
    def __init__(self,states,deltat):
    ...
    def next_state(self,actions):
        ...
        '''
        ego_velocity = self.state[:,-2]
        self.state[:,0] = self.state[:,0] + self.state[:,-2]*self.deltat + 0.5*actions.squeeze()*self.deltat**2
        self.state[:,-2] += actions.squeeze()
        
        self.state[self.state[:,1]>0][:,1]= self.state[self.state[:,1]>0][:,1] - self.state[self.state[:,1]>0][:,-1]*np.sin(self.angle)*self.deltat
        self.state[self.state[:,2]>0][:,1]= self.state[self.state[:,2]>0][:,2] - self.state[self.state[:,2]>0][:,-1]*np.sin(self.angle)*self.deltat
        self.state[self.state[:,2]<=0][:,1]= self.state[self.state[:,2]<=0][:,2] - self.state[self.state[:,2]<=0][:,-1]*np.sin(self.angle)*self.deltat
...
class MBPO:
    @autoassign
    def __init__(self, policy, value_fun, cost_fun, simulator, trajectory_len, uncertainty,eta,safethreshold=8,deltat=0.01,alpha=0.1,max_kl=1e-2,
                 max_val_step=1e-2, max_cost_step=1e-2, max_constraint_val=0.1, val_iters=1, cost_iters=1, val_l2_reg=1e-3,
    ...
    def train(self, n_episodes):
        ...
        while self.episode_num < n_episodes:
            start_time = dt.now()
            self.episode_num += 1
            self.variance.append(self.policy.gaussian.log_std)

            memory,violation = self.simulator.run_sim()
            ...
            # solve a statewise policy optimization
...
#             constraint_cost = torch.mean(torch.tensor([disc_costs[start] for start in trajectory_limits[:-1]]))
            ...
            #constraint_cost should be the expectation of the cbf of the next step. 
    ...
    def update_policy(self, observations, actions, reward_advs, constraint_advs, J_c):
        ...
        forcastor = Next_state(observations,self.deltat)
        expected_h = torch.zeros((observations.shape[0],20))
        J_c = torch.zeros((observations.shape[0],20)).to(self.device)
        deltx = observations[:,0]-observations[:,2]
        deltv=observations[:,-2]-observations[:,-1]
        # compute A and b follows the equation of the paper
        A = -2*(deltx*self.deltat).view(-1,1)
        b = 2*deltx*(deltv)+self.alpha*cbf(observations,self.safethreshold) - scipy.stats.norm.ppf(self.eta)*(deltx*self.uncertainty*deltx).sqrt()
        # bootstrap multiple times to estimate the expectation of E[h(s_{t+1})], use REINFORCE trick to estimate the gradient
        for i in range(20):
            boot_action = action_dists.sample()
            Au = A*boot_action
            val = action_dists.log_prob(boot_action)*Au.squeeze()
            J_c[:, i] = Au.squeeze()
            expected_h[:, i] = val.squeeze()
        expected_h = expected_h.mean(1).reshape(-1,1)
        J_c = J_c.mean(1).view(-1,1) - b
        
        
        constraint_loss = torch.mean(expected_h.to(self.device))

            
            
        # torch.exp(log_action_probs - log_action_probs.detach()) has the same function as REINFORCE trick, just used to compute the gradient
